...
from vss_tools.vspec.model import (
    VSSDataDatatype, get_model, VSSDataBranch, VSSDataSensor,
    NodeType
)
    ...
    def merge(self, other: VSSNode) -> None:
        ...
        for other_node in PreOrderIter(other):
            if "autogenerated" in other_node.data.get_additional_fields():
                log.info(f"Skipping autogenerated node {other_node.name}")
                continue
            ...
            if other_fqn in fqns:
                ...
                for node in PreOrderIter(self):
                    if node.get_fqn() == other_fqn:
                        self.merge_attributes(node, other_node) # type: ignore

            else:
                log.info(f"ToDo: Adding node {other_node.name}")
                # ToDo: Add node
                pass
...
def build_trees(data: dict[str, Any], is_overlay: bool = False) -> tuple[list[VSSNode], list[VSSNode]]:
    ...
    for k, v in data.items():
        ...
        if parent and parent in nodes and not is_overlay:
            ...
            log.warning(f"Parent of {k} not found, so automatically creating to build overlay tree.")
