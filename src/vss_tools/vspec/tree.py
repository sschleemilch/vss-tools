...
def autocreate_branches(nodes: dict[str, VSSNode], parent_path: str) -> None:
    log.info(f"Implicitly creating branch {parent_path}")
    parent = VSSNode(get_name(parent_path), {"type": "branch", "description": "Autogenerated branch", "autogenerated": True})
    ...
    if parent_of_parent and parent_of_parent not in nodes:
        autocreate_branches(nodes, parent_of_parent)


def build_overlay_trees(data: dict[str, Any]) -> list[VSSNode]:
    nodes: dict[str, VSSNode] = {}
    ...
    for k, v in data.items():
        ...
        if parent and parent in nodes:
            ...
            log.warning(f"Parent of {k} not found, so automatically creating it.")
            autocreate_branches(nodes, parent)
        else:
            for child_id in find_children_ids(list(nodes.keys()), k):
                node.children.append(nodes[child_id])
        # breakpoint()
